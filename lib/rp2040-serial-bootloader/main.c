...
static enum state state_wait_for_sync(struct cmd_context *ctx)
{
	...
	while (idx < sizeof(ctx->opcode)) {
		uart_read_blocking(ESP_UART, &recv[idx], 1);
...
{
	uart_read_blocking(ESP_UART, (uint8_t *)&ctx->opcode, sizeof(ctx->opcode));
...
static enum state state_read_args(struct cmd_context *ctx)
{
	...
	uart_read_blocking(ESP_UART, (uint8_t *)ctx->args, sizeof(*ctx->args) * desc->nargs);
...
static enum state state_read_data(struct cmd_context *ctx)
{
	...
	uart_read_blocking(ESP_UART, (uint8_t *)ctx->data, ctx->data_len);
...
static enum state state_handle_data(struct cmd_context *ctx)
{
	...
	uart_write_blocking(ESP_UART, ctx->uart_buf, resp_len);
...
static bool should_stay_in_bootloader()
{
	...
	return !gpio_get(SERIAL_BOOT) || wd_says_so;
}

int main(void)
{
    gpio_init(SERIAL_BOOT);
	gpio_pull_up(SERIAL_BOOT);
	gpio_set_dir(SERIAL_BOOT, 0);
    ...
    const char* ready_msg = "Ready for firmware upload";

    uart_write_blocking(ESP_UART, ready_msg, strlen(ready_msg));
